  ...
  const content = (
    <>
      ...
      <div className="bento-card__body">
        <span className="bento-card__index">
          {item.index}
        </span>
...
/**
 * MagicBento
 * Reusable asymmetric service card grid.
 *
 * If item.link exists:
 * → card becomes a React Router Link.
 *
 * If item.link does not exist:
 * → existing behaviour remains unchanged.
 */
export default function MagicBento({ items, variant = "default" }) {
  const spans = ['bento-card--wide', '', '', '', 'bento-card--tall', ''];

  return (
    <div className={`magic-bento ${variant === "home" ? "magic-bento--home" : ""}`}>
      {items.map((item, i) => (
        <BentoCard
          key={item.title}
          item={item}
